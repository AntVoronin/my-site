window.addEventListener('DOMContentLoaded', function() {
    ...
    const menu_str   = '.sidebar_menu a',
          content_str= '.content',
          submenu_str= '[data-sub_menu="sub_menu"]',
          logo_str   = '.sidebar_logo';
    const menu       = document.querySelectorAll(menu_str),
          content    = document.querySelector(content_str),
          li_submenu = document.querySelectorAll(submenu_str),
          logo       = this.document.querySelector(logo_str);
    ...
    menu.forEach( el=> {
        el.addEventListener('click', event=> {
            ...
            switch(el.id) {           
                ...
                case 'cv':  
                innerHTMLitem(content, str.cvHtml);
                break

                case 'contact': 
                innerHTMLitem(content, str.contactHtml);
                break
    ...
    function gallery(el) {
        content.innerHTML = '';
        //сделать ф-цией??
        content.insertAdjacentHTML("beforeend", `<div class="wrap_text"> <p>${str.textAbout[el.id]}</p> </div>` );
